...
document.addEventListener('DOMContentLoaded', async () => {
  try {
    ...
    allUsersIds = sortedUsers

    renderUsersTable(allUsersIds);
  } catch (err) {
...
function renderUsersTable(users) {
  ...
  users.forEach(user => {
    ...
    entry.innerHTML = `
      <div><strong class = "onscreenText adminhtmlNumber:"></strong><p class = "onscreenText${user.id}" style="display: inline;"></p></div>
      <div><strong class = "onscreenText adminhtmlFullName:"></strong><p class = "onscreenText${user.id}" style="display: inline;" onclick="this.focus()" ></p></div>
      <div><strong class = "onscreenText adminhtmlEmail:"></strong><p class = "onscreenText${user.id}" style="display: inline;" onclick="this.focus()" ></p></div>
      <div><strong class = "onscreenText adminhtmlPassword:"></strong><p class = "onscreenText${user.id}" style="display: inline;" onclick="this.focus()" ></div>
      <div><strong class = "onscreenText adminhtmlEnabled:"></strong><p class = "onscreenText${user.id}" style="display: inline;" onclick="this.focus()" ></div>
...
let allUsersContents = [];
document.getElementById('submitToggleAllUsers').addEventListener('click', async () => {
  const wrapper = document.getElementById('userTableWrapper');
    ...
    allUsersContents = sortedUsers

    fillUsersTable(allUsersContents);
  } catch (err) {
    console.error('Error fetching users:', err);
    alert('Could not load users.');
  }
...
function fillUsersTable(users) {
  for (i=0; i<users.length; i++){
    const a="onscreenText"+users[i].id
    document.getElementsByClassName(a)[0].innerText = users[i].id
    document.getElementsByClassName(a)[1].innerText = users[i].name
    document.getElementsByClassName(a)[2].innerText = users[i].email
    document.getElementsByClassName(a)[3].innerText = users[i].password
    document.getElementsByClassName(a)[4].innerText = users[i].enabled
  }
}
...
function cancelUserEdition(userId, userName, userEmail, userPassword, userEnabled) {
  ...
  document.getElementsByClassName(onscreenUserId)[1].innerText = userName
  document.getElementsByClassName(onscreenUserId)[2].innerText = userEmail
  document.getElementsByClassName(onscreenUserId)[3].innerText = userPassword
  document.getElementsByClassName(onscreenUserId)[4].innerText = userEnabled
...
async function saveUserEdition(userId) {
  ...
  // changing user - start
    const userFormData = {
      name: document.getElementsByClassName(onscreenUserId)[1].innerText,
      email: document.getElementsByClassName(onscreenUserId)[2].innerText,
      password: document.getElementsByClassName(onscreenUserId)[3].innerText,
      enabled: document.getElementsByClassName(onscreenUserId)[4].innerText,
      id: userId,
    };
